...
<%@page import="in.org.cris.mrsectt.dao.DashboardSubjectWiseDAO"%>
...
<SCRIPT src='${pageContext.request.contextPath}/script/validateinput.js'></SCRIPT>
...
ArrayList<CommonBean> RefDashBoardDetails = new DashboardSubjectWiseDAO().getRefDashBoardDetailListHOD(subjectCode,loginID,status);
...
String queryRailwayCode ="SELECT SNAME, FNAME, RLYCODE FROM MSTRLY order by SNAME";
ArrayList<CommonBean> railwayList = (new CommonDAO()).getSQLResult(queryRailwayCode, 3);
...
function SaveCompliance(refid,sno,loginIDName){
	//console.log(refid + ":::::::::::::::" + sno);
	var compliianceRem = window.document.getElementById("complianceRemarks_"+sno).value;
//	console.log(window.document.getElementById("COMPLIANCE_"+sno));
	var compliance = window.document.getElementById("COMPLIANCE_"+sno).value;
	var forwardTo = window.document.getElementById("FORWARD_TO_"+sno).value;
	var railwayTo = window.document.getElementById("RAILWAY_TO_"+sno).value;
	
	//alert (compliianceRem +"=========" +compliance);
	DashboardSubjectWiseDAO.saveComplianceData(refid,compliance,compliianceRem,loginIDName,forwardTo,railwayTo,dateSaveCompliance);
...
<!-- CHANGED ON 24_07_2012 -->
			<table width="100%" style="margin-top: 1px;margin: 1px;" border="1" id="example">
				...
				<td style="border-bottom: dotted; border-width: 1px; border-color: blue"><b>Remarks <br>Signed By<br>Signed On</b></td>
					...
					<td style="border-bottom: dotted; border-width: 1px; border-color: blue"><b>Railway</b></td>
				...
				<%
					for(int i=0;i<RefDashBoardDetails.size();i++){
						...
						<td>
						<select name="RAILWAY_TO_<%=i+1%>" style="width: 120px" tabindex="21"> 	 
							<option value="" selected>Select one- </option>
								<%
									for(int k=0;k<railwayList.size();k++){
									CommonBean beanCommon = (CommonBean) railwayList.get(k);
									%>
								<option value="<%=beanCommon.getField1()%>" <%=beanCommon.getField1().equalsIgnoreCase(bn.getField20())?"selected":""%>><%=beanCommon.getField2()%></option>
								<%
								}%>
							</select>
						</td>
						...
						<select name="FORWARD_TO_<%=i+1%>" style="width: 120px" tabindex="21"> 	 
							<option value="" selected>Select one- </option>
								<%
									...
									%>
								<option value="<%=beanCommon.getField1()%>" <%=beanCommon.getField1().equalsIgnoreCase(bn.getField19())?"selected":""%>><%=beanCommon.getField1()%></option>
								<%
								}%>
							</select>
