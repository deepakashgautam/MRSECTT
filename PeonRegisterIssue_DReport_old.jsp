...
<%
    ...
    {
	dataArr = (new BudgetReportDAO()).getPeonReportData_IssueD(sessionBean.getLOGINASROLEID(),sessionBean.getTENUREID(),BOOKDATEFROM,BOOKDATETO,CLASS,sessionBean.getISCONF()); 
				...
				<td align="center"><table width="984px" border="0"  cellspacing="0"
					...
					<tr>
						...
						<br><br><center><fieldset style="width: 60%;">		
								...
								<td nowrap="nowrap" align="left"><input type="text" name="BOOKDATEFROM" id="BOOKDATEFROM" value='<%=request.getParameter("BOOKDATEFROM")!=null ? request.getParameter("BOOKDATEFROM"):serverDate%>'/></td>
									<td> <b>&nbsp - &nbsp</b> </td>
								<td nowrap="nowrap" align="left"><input type="text" name="BOOKDATETO" id="BOOKDATETO" value='<%=request.getParameter("BOOKDATETO")!=null ? request.getParameter("BOOKDATETO"):serverDate%>'/></td>
						...
						<table width="80%" class="tablesorter" id="sorttable"
							...
							<thead>
								...
								<tr>
									...
									<table>
										<tr>
											<td nowrap="nowrap" style="text-align: left;" width="442"><HR>PEON BOOK FOR DT:<%=request.getParameter("BOOKDATEFROM")!=null ? request.getParameter("BOOKDATEFROM"):"" %><br />
									Class :  <%=request.getParameter("CLASS").length()>0 ? request.getParameter("CLASS"): "ALL" %><HR></td>
											<td style="text-align: right" width="330"><HR> <%=serverDate %><br /> <%=serverTime %>
									<HR></td>
										</tr>
									</table>
									</td>
...
//							CommonBean bean = (CommonBean) dataArr.get(i);
							%>
								<tr class="trodd">
									<td><%= i<dataArr.size()?((1+i)+".  "+dataArr.get(i++).getField1()):"" %></td>
									<td><%= i<dataArr.size()?((1+i)+".  "+dataArr.get(i++).getField1()):"" %></td>
									<td><%= i<dataArr.size()?((1+i)+".  "+dataArr.get(i++).getField1()):"" %></td>
								</tr>
							<%} %>
