...
<script>
 ...
 //	window.location.href="Report"+reptype+".jsp?reportid="+repid+"&"+param;
	window.location.href="ReportFileMov.jsp?reportidencr="+repid+"&"+param;
 }
 
 function runReport(repid,repType){
 	window.location.href="GenerateReportFileMovController?save=Y&generate=Y&reportidencr="+repid+"&repTypeenc="+repType;
 }
        ...
        <td align="center" style="padding-left:7px;"><br/><br/>
			<table  width="40%" border="1" bordercolor="#7e7d7d" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
                  	...
                  	<tr>
                  		...
                  		<td><c:out value='<%=  URLDecoder.decode(reparr.get(i).getField()[1], "utf-8")   %>'/></td>
                  		<td><c:out value='<%= reparr.get(i).getField()[2] %>'/></td>
                  		 <c:set var="repidenc" value="<%= reparr.get(i).getField()[0] %>"/>
                  		<c:set var = "reptypeenc" value ="<%=reparr.get(i).getField()[5] %>"/>
						<td align="center" style="white-space: nowrap;">     
							<input type="button" class="ui-icon ui-icon-play" style="width: 20px; height: 20px; cursor: pointer;" value=" "  onclick="runReport('<c:out value="${enc:encryptInt(repidenc)}"/>','<c:out value="${enc:encryptInt(reptypeenc)}"/>');" title="Run Report">
						</td>
						<td align="center">
    						<input type="button" class="ui-icon ui-icon-wrench" style="width: 20px; height: 20px; cursor: pointer;"  value=" " onclick="openReport('<c:out value="${enc:encryptInt(repidenc)}"/>','<c:out value="<%=reparr.get(i).getField()[4]%>"/>','<c:out value="<%= reparr.get(i).getField()[5] %>"/>');" title="Edit Report" />
                  		</td>
<!--              		<td align="center">
                  			<input type="button"  class="ui-icon ui-icon-copy" style="width: 20px; height: 20px; cursor: pointer;" value=" " onclick="openReport('','<c:out value="<%=reparr.get(i).getField()[4]%>"/>','','<c:out value="<%= reparr.get(i).getField()[5] %>"/>');" title="Copy as New Report" />
                  		</td> -->
                  		<td align="center">
                  			<input type="button"  class="ui-icon ui-icon-closethick" style="width: 20px; height: 20px; cursor: pointer;" name="delbtn" value=" Delete " onclick="deleteReport(this,'<c:out value="<%=reparr.get(i).getField()[0]%>"/>');" title="Delete Report" />
                  		</td>
                  	</tr>
                 		 <%} %>
                 		 </tbody>
