...
function getSearch(thisObj){
 ...
 //Check for atleast One field for search
  ...
  function getData(data){
	...
	var attachment = (data[loop].ISATTACHMENT == "1"? '<img border="0" alt="Image Gallery" src="${pageContext.request.contextPath}/images/stn.gif" onclick="popupGallery('+data[loop].FMID+','+data[loop].FMID+');">': "");
	...
	var refClass = data[loop].REFERENCETYPE == 'C'? '<font color="#FF0000">'+data[loop].REGISTRATIONNO+'*</font>': data[loop].REGISTRATIONNO;
		htmlText += '<tr style="cursor: pointer;" onclick="maximizeResult('+data[loop].FMID+')"><td align="center" id="std00" style="border-bottom: dotted; border-width: 1px; border-color: gray;"> '+(loop+1)+' </td>  <td  id="std20" style=" border-bottom: dotted; border-width: 1px; border-color: gray;"> '+attachment+'&nbsp; </td> <td align="left" id="std0" style="border-bottom: dotted; border-width: 1px; border-color: gray;">'+refClass+' </td><td id="std1" style="border-bottom: dotted; border-width: 1px; border-color: gray;"> '+data[loop].REGISTRATIONDATE+' </td><td style="display:none; border-bottom: dotted; border-width: 1px; border-color: gray;" id="std2"> '+data[loop].FILENO+' </td><td style="display:none; border-bottom: dotted; border-width: 1px; border-color: gray;" id="std3"> '+data[loop].FILESUBJECTCODE+' </td><td id="std31" style="display:none; border-bottom: dotted; border-width: 1px; border-color: gray;" > '+data[loop].SUBJECT+' </td><td style="display:none; border-bottom: dotted; border-width: 1px; border-color: gray;" id="std4"> '+data[loop].IMARKINGTO+' </td><td style="display:none; border-bottom: dotted; border-width: 1px; border-color: gray;" id="std5"> '+data[loop].FILESTATUS1+' </td><td style="display:none; border-bottom: dotted; border-width: 1px; border-color: gray;" id="std6"> '+data[loop].FILESTATUS2+' </td><td style="display:none; border-bottom: dotted; border-width: 1px; border-color: gray;" id="std7"> '+data[loop].DMARKINGTO+' </td><td style="display:none; border-bottom: dotted; border-width: 1px; border-color: gray;" id="std8"> '+data[loop].DMARKINGDATE+' </td> <td style="display:none; border-bottom: dotted; border-width: 1px; border-color: gray;" id="std9"> &nbsp; </td></tr>';
	}
...
function showSuggestion(){
	...
	for(var i=1;i<=objArray.length-1;i++){
			...
			if(x=="D"){
					...
					//document.getElementsByName("sugcomremarksc").style.display="block";
			...
			<div id="tabs" style="border: 0px">
			    ...
			    <div id="File" >
			    	...
			    	<table id="data" width="1000"><tr class="trodd"><td>Regn. No.</td>
							...
							<INPUT 	type="button" class="calbutt" name="regdt" onclick="newCalendar('REGISTRATIONDATE-L');"><input type="hidden" name="FMID" id="FMID" value="<c:out value='<%= fileBean.getFMID()%>'/>" readonly="readonly" class="readonly">
				...
				<%} %>
			    		...
			    		<tr class="trodd">
							...
							<td colspan="2"><input type="text" id="FILENO" onkeydown="changecase(this,'U')" size="40" name="FILENO" tabindex="1" value="<c:out value='<%= fileBean.getFILENO()%>'/>" onblur="getFileSearch(this);setOraignatedAt(this);" onkeypress="allowAlphaNum('()&/*-');changecase(this,'U')">
					...
					TrnFileRef refBean = (TrnFileRef)fileBean.getRefBeanList().get(i);
						...
						name="REFNO" id="REFNO" value="<%=refBean.getREFNO()+" ("+refBean.getINCOMINGDATE()+")" %>" style=" background-image: url('${pageContext.request.contextPath}/images/search.png'); background-position: right;background-repeat: no-repeat; ">
										...
										<td nowrap="nowrap" id="REFERENCENAMEV" width="100px;"><nobr><c:out value='<%=refBean.getREFERENCENAME() %>'/><br><c:out value='<%=refBean.getVIPSTATUS() %>'/>,<c:out value='<%=refBean.getSTATENAME() %>'/>&nbsp;</nobr></td>
										<td nowrap="nowrap" id="SUBJECTCODEV" width="50px;"><nobr><c:out value='<%=refBean.getSUBJECTCODE() %>'/>&nbsp;</nobr></td>
										<td id="SUBJECTV" ><c:out value='<%=refBean.getSUBJECT() %>'/>&nbsp;</td>
										<td nowrap="nowrap" id="STATUSV"><nobr><c:out value='<%=refBean.getSTATUS() %>'/>&nbsp;</nobr></td>
										<td nowrap="nowrap">
					...
					TrnFileIntMarking intmarkingBean = (TrnFileIntMarking)fileBean.getIntmarkingBeanList().get(i);
						...
						<%
							...
							<td nowrap="nowrap" ><input type="hidden"
											...
											name="IMARKINGDATE" id="IMARKINGDATE" size="12" maxlength="10"
											value="<c:out value='<%= intmarkingBean.getMARKINGDATE().equalsIgnoreCase("")?serverDate:intmarkingBean.getMARKINGDATE()%>'/>"   onblur="chkWorkDate(this);"><input type="button" class="calbutt" name="imrkdt"
											onclick="newCalendar('IMARKINGDATE-L',getIndex(this));" ></td>
							<td nowrap="nowrap" ><input type="text"
											name="IRETURNDATE" id="IRETURNDATE" size="12" maxlength="10"  
											value="<c:out value='<%= intmarkingBean.getCHANGEDATE()%>'/>"  onblur="chkWorkDate(this);"><input type="button" class="calbutt" name="iretdt"
											onclick="newCalendar('IRETURNDATE-L',getIndex(this));"></td>
							<td width="145" style="display: none" >
										<textarea rows="2" cols="20" name="IMARKINGREMARK" id="IMARKINGREMARK"  tabindex="11"></textarea></td>
			    	...
			    	<tr class="trodd">
						<td nowrap="nowrap" width="115"><select name="DMARKINGTO" id="DMARKINGTO"
							...
							<td nowrap="nowrap" width="121"><nobr><input type="text"
											...
											value="<c:out value='<%=markingBeanD.getMARKINGSEQUENCE()%>'/>"><input
...
<%	if(msg.length()>3) { %>
			...
			<BR><img src="images/<c:out value='<%= msg.substring(0,3)%>'/>.gif"/><c:out value='<%= msg.substring(3) %>' escapeXml="false"/> <BR><input type="button" value=" Ok " onclick="callMe()" id="msgok">
			<BR><BR>
