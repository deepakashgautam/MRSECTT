...
public class UploadServ extends HttpServlet
{
	...
	static Logger log = LogManager.getLogger(UploadServ.class);
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
	{
		...
		{   
			con.openConnection();
			String LOGINDESIGNATION =sessionBean.getLOGINDESIGNATION(); 
			String LOGINID = sessionBean.getLOGINID(); 
			int delpermission= 0;
			if(LOGINDESIGNATION.equalsIgnoreCase("DD/MRCC")){delpermission = 1;}
			else if (LOGINDESIGNATION.equalsIgnoreCase("SO/MRCC")){delpermission = 1;}
			else if (LOGINDESIGNATION.equalsIgnoreCase("admin")){delpermission = 1;}
			else {delpermission = 0;}
			
			if ((delpermission==1)||(( LOGINID== "alok"))){
				
				
				String strSQL = " UPDATE TRNATTACHMENT SET STATUSFLAG='0'" +
								" WHERE SUBSTR(NEWFILENAME,0,LENGTH(NEWFILENAME)-4) = ? AND TYPE=? ";
