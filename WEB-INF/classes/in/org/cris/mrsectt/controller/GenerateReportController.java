...
//System.out.println("reportid :" + reportid);
		...
		//System.out.println("reportidencr :" + reportidencr);
		...
		String repTypeenc = request.getParameter("repTypeenc") != null ? Encrypt.decrypt(request.getParameter("repTypeenc")) : "1";
...
//			String[] selcolval = request.getParameterValues("sel_tabcols") != null ? request.getParameterValues("sel_tabcols"): new String[0];
			...
			String condVIPPARTY="";
			for (int i = 0; i<VIPPARTY.length;i++){
				condVIPPARTY += ",'" + VIPPARTY[i] + "'";
			}
			if (VIPPARTY.length == 1)
				condVIPPARTY = " AND TRNREFERENCE.VIPPARTY = "+condVIPPARTY.substring(1)+" ";
			else if (VIPPARTY.length > 0)
				condVIPPARTY = " AND TRNREFERENCE.VIPPARTY IN ("+condVIPPARTY.substring(1)+") ";
			//System.out.println("condvipparty "+condVIPPARTY);
			...
			LANGUAGE = LANGUAGE.length() > 0 ? " AND TRNREFERENCE.LANGUAGE = '"+LANGUAGE+"' " : "";
			...
			// hm.put("7", "TRNREFERENCE.STATECODE");
			 hm.put("7", "(SELECT STATENAME FROM MSTSTATE WHERE STATECODE=TRNREFERENCE.STATECODE)");
			...
			//======
			 ...
			 //=====
			 hm.put("44", "(SELECT N.FNAME FROM MSTSUBJECT M, MSTGEC N  WHERE  M.SUBJECTCODE=TRNREFERENCE.SUBJECTCODE AND M.SUBJECTCATEGORY = N.CODE AND N.CODETYPE = '12' AND N.CODE<>'00')");
			 hm.put("45",  "TRNREFERENCE.VIPSTATUS||'\n'||(SELECT STATENAME FROM MSTSTATE WHERE STATECODE=TRNREFERENCE.STATECODE)||'\n'||TRNREFERENCE.VIPPARTY||'\n'||TRNREFERENCE.VIPCONSTITUENCY");
			 hm.put("46", "TRNREFERENCE.LETTERNO");
			...
			FinalSelection = "SELECT DISTINCT TRNREFERENCE.REFID \"REFID\", " +  FinalSelection.substring(1)+ " ,trnreference.refid, NVL((SELECT DISTINCT '1' FROM TRNATTACHMENT M WHERE M.REFID = TRNREFERENCE.REFID AND M.TYPE='R'),'0') ISATTACHMENT ";
			String conditions = condREFCLASS + condVIPTYPE+ condVIPPARTY + condCOMPLIANCE+condREFCATEGORY + condSUBJECTTYPE + condMARKTO + ROLEID + REFCONDITION + BUDGETYEAR +LANGUAGE+ INDATECONDITION + LETDATECONDITION + RECEIVEDFROM + STATUS + STATE + SUBJECT + ACKNBY + ACKNDATECONDITION + MARKINGDATECONDITION + TARGETDATECONDITION + REMARKS + SIGNEDBY + SIGNEDONCONDITION + PLCRLYCONDITION + TAG + RLY + PLACE ; 
		...
		if (repExportType.equals("E")) {
			...
			//System.out.println("here 1"+RepPath);
