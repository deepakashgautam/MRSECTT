...
public class TrnFileManagementDAO {
	static Logger log = LogManager.getLogger(TrnFileManagementDAO.class);
...
*/		tempSQL += isConf.equalsIgnoreCase("0")? " AND ISCONF = '0'": " AND ISCONF IN ('0','1')";
		...
		try {
			...
			while (rs.next()) {
				...
				//System.out.print("FMID "+bn.getFMIDENC());
		...
		try {
			...
			if(sString.length() > 4 && sString.substring(0, 5).equalsIgnoreCase("USER ")){
				...
				while (rs.next()) {
					...
					bn.setFMID(StringFormat.nullString(rs.getString("FMID")));
					bn.setPuclistenc(Encrypt.encrypt(StringFormat.nullString(rs.getString("PUCLIST"))));
					bn.setFmidencr(Encrypt.encrypt(StringFormat.nullString(rs.getString("FMID"))));
					...
					bn.setDMARKINGDATE(StringFormat.nullString(rs.getString("DMARKINGDATE")));
					//System.out.println(rs.getString("REFERENCETYPE"));
					if (rs.getString("REFERENCETYPE").equalsIgnoreCase("Confidential")){
						
						bn.setMARKINGREMARK(StringFormat.nullString(Encrypt.decrypt(StringFormat.nullString(rs.getString("MARKINGREMARK")))));
					}
					else{
		...
		try {
			...
			if(sString.length() > 4 && sString.substring(0, 5).equalsIgnoreCase("USER ")){
				...
				String strSQL = " SELECT DISTINCT A.FMID, "+Qreply+" ISATTACHMENT," +
					...
					" MARKINGLIST,1 CNT,"+Freply+" REPLY " +
					" FROM TRNFILEHDR A, TRNFILEMARKING B" +
					" WHERE 1=1 AND A.FMID = B.FMID(+)  AND DECODE(A.DESTINATIONMARKING,?,A.ROLEIDDES,A.ROLEIDORG) = ?" +
					" "+condSub+"" +
			...
			if(advtext.equalsIgnoreCase("0")){
				...
				PreparedStatement ps = null;
				 ...
				 condConf += isConf.equalsIgnoreCase("0")? " AND ISCONF = '0'": " AND ISCONF IN ('0','1')";
						...
						try{dbCon.openConnection();
						ps = dbCon.setPrepStatement(strSQL);
						ps.setString(1, roleId);
						ps.setString(2, roleId);
						ps.setString(3, roleId);
						ps.setString(4, roleId);
						ps.setString(5, sString);
						ps.setString(6, roleId);
						ps.setString(7, fdate);
						ps.setString(8, tdate);
...
*/
		...
		String strSQL = " SELECT DISTINCT A.FMID, ? ISATTACHMENT," +
						" DECODE(A.DESTINATIONMARKING,?,A.REGISTRATIONNODES,A.REGISTRATIONNOORG) REGISTRATIONNO," +
						" TO_CHAR(DECODE(A.DESTINATIONMARKING,?,A.REGISTRATIONDATEDES,A.REGISTRATIONDATEORG),'DD/MM/YYYY') REGISTRATIONDATE," +
						" CASE WHEN REFERENCETYPE = 'D' THEN 'Draft' WHEN REFERENCETYPE = 'A' THEN 'Approval' WHEN REFERENCETYPE = 'P' THEN 'Position' WHEN REFERENCETYPE = 'C' THEN 'Confidential' END REFERENCETYPE," +
						" A.FILENO,(SELECT LISTAGG((SELECT REFNO||'   '||TO_CHAR(INCOMINGDATE,'DD/MM/YYYY') FROM TRNREFERENCE WHERE REFID=P.REFID), ',') WITHIN GROUP (ORDER BY REFID) FROM TRNFILEREF P WHERE P.FMID=A.FMID) PUCLIST," +
			...
			try	{
				dbCon.openConnection();
				ps = dbCon.setPrepStatement(strSQL);
				ps.setString(1,Qreply);
				ps.setString(2,roleId);
				ps.setString(3,roleId);
				ps.setString(4,Freply);
				ps.setString(5,roleId);
				ps.setString(6,roleId);
				//ps.setString(7, sString);
				
				//System.out.println("Params size ="+params.size());
				for(int i=0;i<params.size();i++){
					//System.out.println("Value of i ="+i);
					
					ps.setString(i+7, params.get(i));
				}
...
*/
	...
	public TrnFileHdr getFileData(String fmId, String roleId, String isReply) throws UnsupportedEncodingException {
		//System.out.println("im in controller GO");
		...
		try {
			...
			String strSQL = " SELECT FMID,TO_CHAR(REGISTRATIONDATEORG,'DD/MM/YYYY') REGISTRATIONDATEORG,REFERENCETYPE,FILENO,DEPARTMENTCODE,FILESTATUS1,FILESTATUS2,SENTTO,NOL,DESTINATIONMARKING,"
					...
					+ "TENUREID,REPLYTYPE,REASON,SUBJECT,REGISTRATIONNODES,REGISTRATIONNOORG,PROPOSEDPATH, ISBUDGETPOINT,BUDGETPOINTVIPID,NVL((SELECT VIPNAME FROM MSTVIP M WHERE M.VIPID=BUDGETPOINTVIPID),'') BUDGETPOINTVIPPERSON FROM TRNFILEHDR WHERE FMID = ? ORDER BY FMID ";
			...
			while (rs.next()) {
				...
				filetype=StringFormat.nullString(rs
						.getString("REFERENCETYPE"));
				...
				fileBean.setISBUDGETPOINT(StringFormat.nullString(rs
						.getString("ISBUDGETPOINT")));
				fileBean.setBUDGETPOINTVIPID(StringFormat.nullString(rs
						.getString("BUDGETPOINTVIPID")));
				fileBean.setBUDGETPOINTVIPPERSON(StringFormat.nullString(rs
						.getString("BUDGETPOINTVIPPERSON")));
		...
		rsReply.close();
			...
			// Getting the Detail Marking Table Details
			strSQLMarking = " SELECT FMID, MARKINGSEQUENCE, MARKINGFROM, MARKINGTO, TO_CHAR(MARKINGDATE, 'DD/MM/YYYY') MARKINGDATE, MARKINGREMARK, "
					+ " SUBJECTCODE, SUBJECT,MARKINGTYPE, STATUSCHANGEUSER "
					+ " FROM TRNFILEMARKING WHERE FMID =?  AND MARKINGSEQUENCE =1 "
			...
			while (rsMarking1.next()) {
				...
				markingBean.setMARKINGDATE(StringFormat.nullString(rsMarking1
				 ...
				 if(filetype.equalsIgnoreCase("C"))
					{
					 markingBean.setMARKINGREMARK(StringFormat.nullString(Encrypt.decrypt(StringFormat.nullString(rsMarking1.getString("MARKINGREMARK")))));
				...
				markingBean.setMARKINGTYPE(StringFormat.nullString(rsMarking1
						.getString("MARKINGTYPE")));
				markingBean.setSTATUSCHANGEUSER(StringFormat.nullString(rsMarking1
						.getString("STATUSCHANGEUSER")));
	...
	public String saveReference(TrnFileHdr trnfileBean,TrnFileReply trnfileReply,
			...
			ArrayList<TrnFileMarking> trnMarkingBeanDList) throws UnsupportedEncodingException {
		...
		try {
			...
			if (rec0 == 1) {
				...
				params.add(trnfileBean.getREPLYTYPE());
						 ...
						 params.add(trnfileBean.getISBUDGETPOINT());
						 params.add(trnfileBean.getBUDGETPOINTVIPID());
			...
			if (rec0 == 0) {
				...
				String isconf = "0";
				isconf = trnfileBean.getREFERENCETYPE().equalsIgnoreCase("C")?"1":"0";
				...
				String fileQuery = " INSERT INTO TRNFILEHDR(FMID,REGISTRATIONDATEORG,REFERENCETYPE,FILENO,DEPARTMENTCODE,FILESTATUS1,"
						...
						+ "REPLYTYPE,REASON,SUBJECT,REGISTRATIONNODES,REGISTRATIONNOORG,PROPOSEDPATH,ISCONF,ISBUDGETPOINT,BUDGETPOINTVIPID)"
						+ " VALUES (?,"
						+ " TO_DATE(?, 'DD/MM/YYYY'),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,"
						+ " SYSDATE,"
						+ " TO_DATE(?, 'DD/MM/YYYY'),?,?,?,?,?,?,?,?,?,?,?,?,?)";
				...
				ps.setString(1,trnfileBean.getFMID());
				 ...
				 ps.setString(33,isconf);
				 ps.setString(34,trnfileBean.getISBUDGETPOINT());
				 ps.setString(35,trnfileBean.getBUDGETPOINTVIPID());
		...
		//reply data ends 
			...
			for (int i = 0; i < trnMarkingBeanDList.size(); i++) {
				...
				if (rec == 1) {
					...
					if(trnfileBean.getREFERENCETYPE().equalsIgnoreCase("C"))
					{
						ps.setString(3,Encrypt.encrypt(StringFormat.nullString(trnMarkingDBean.getMARKINGREMARK())) );
					}
					else{
				...
				if (rec == 0) {
					...
					String markingUQuery = " INSERT INTO TRNFILEMARKING (FMID, MARKINGSEQUENCE, MARKINGFROM, MARKINGTO, MARKINGDATE, MARKINGREMARK, "
							+ " SUBJECTCODE, SUBJECT, ACTION, ACTIONDATE,MARKINGTYPE, STATUSCHANGEUSER) "
							+ " VALUES (?,?,?,?,"
							+ " SYSDATE,"
							+ " ?,?,?, 'FOW', SYSDATE,'D', ? )";
					String confremarkp = "";
					ps = dbcon.setPrepStatement(markingUQuery);
					 ...
					 if(trnfileBean.getREFERENCETYPE().equalsIgnoreCase("C"))
						{
							ps.setString(5,Encrypt.encrypt(StringFormat.nullString(trnMarkingDBean.getMARKINGREMARK())));
						}
						else{
					...
					// ps.setString(5,trnMarkingDBean.getMARKINGREMARK());
					 ...
					 ps.setString(8,trnfileBean.getLOGINID());
			...
			refNoAndMarking +=   trnfileBean.getREGISTRATIONNO()
					 +"/-"+ trnfileBean.getFMID()+"/-"+ trnfileBean.getFILESTATUS1();
	...
	public void UpdateReference(DBConnection con, String fmid, String filecounter, String registrationno, String fileno, String filestatus1,
		...
		String strSQL = " UPDATE TRNREFERENCE A SET A.FMID = ?,A.FILECOUNTER =?, A.REGISTRATIONNO = ?," +
				...
				" AND NVL(A.FILESTATUS1,'0') NOT IN ('5','8','16','13','18','98')";
		

		String strSQL1 = " UPDATE TRNREFERENCE A SET A.FMID = ?,A.FILECOUNTER =?, A.REGISTRATIONNO = ?," +
				" A.FILENO = ?," +
				" A.IMARKINGTO = (SELECT O.MARKINGTO FROM TRNFILEINTMARKING O WHERE O.FMID=? AND O.MARKINGSEQUENCE = (SELECT MAX(P.MARKINGSEQUENCE) FROM TRNFILEINTMARKING P WHERE P.FMID=?))," +
				" A.FILESTATUS2 = ?,A.REPLYTYPE = ?," +
				" A.REASON = ?, A.DMARKINGTO = (SELECT O.MARKINGTO FROM TRNFILEMARKING O WHERE O.FMID=?" +
				" AND O.MARKINGSEQUENCE = 1), A.DMARKINGDATE = (SELECT O.MARKINGDATE FROM TRNFILEMARKING O WHERE O.FMID=?" +
				" AND O.MARKINGSEQUENCE = 1)," +
				" A.REGISTRATIONDATE = TO_DATE(?,'DD/MM/YYYY'), A.STATUSREMARK = ?, "	+
				...
				" A.ICHANGEDATE = (SELECT TO_DATE(O.CHANGEDATE,'DD/MM/YYYY') FROM TRNFILEINTMARKING O WHERE O.FMID=? AND O.MARKINGSEQUENCE = (SELECT MAX(P.MARKINGSEQUENCE) FROM TRNFILEINTMARKING P WHERE P.FMID=?)) " +
				" WHERE A.REFID IN (SELECT O.REFID FROM TRNFILEREF O WHERE O.FMID=?)" +
				" AND NVL(A.FILESTATUS1,'0')  IN ('18','98')";
		...
		try {
			...
			PreparedStatement ps3 = con.setPrepStatement(strSQL1);
			ps3.setString(1,fmid);
			ps3.setString(2,filecounter);
			ps3.setString(3,registrationno);
			ps3.setString(4,fileno);
			ps3.setString(5,fmid);
			ps3.setString(6,fmid);
			 
			ps3.setString(7,filestatus2);
			ps3.setString(8,replytype);
			ps3.setString(9,reason);
			ps3.setString(10,fmid);
			ps3.setString(11,fmid);
			ps3.setString(12,registrationdate);
			ps3.setString(13,remarks);
			ps3.setString(14,fmid);
			ps3.setString(15,fmid);
			ps3.setString(16,fmid);
			ps3.setString(17,fmid);
			ps3.setString(18,fmid);
