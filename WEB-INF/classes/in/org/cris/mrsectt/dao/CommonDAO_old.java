...
public class CommonDAO_old {
	static Logger log = LogManager.getLogger(CommonDAO_old.class);
...
//			System.out.println(strSQL);
	...
	public static String getStateName(String stateCode) {
		...
		try {
			con.openConnection();
			strSQL = "SELECT STATENAME FROM MSTSTATE WHERE STATECODE LIKE '"+stateCode+"'";
			ResultSet rs = con.select(strSQL);
			if (rs.next()) {
	...
	public String changePassword(String loginId, String roleId, String oldPass, String newPass) {
		...
		DBConnection con = new DBConnection();
		try {
			String strSQL = " UPDATE MSTLOGIN SET PASSWORD = '"+newPass+"' WHERE UPPER(PASSWORD) = UPPER('"+oldPass+"') AND UPPER(LOGINID) = UPPER('"+loginId+"') AND LOGINASROLEID = '"+roleId+"'";
			con.openConnection();
			con.update(strSQL);
			con.commit();

			strSQL = " SELECT UPPER(PASSWORD) RES FROM MSTLOGIN WHERE UPPER(PASSWORD) = UPPER('"+newPass+"') AND UPPER(LOGINID) = UPPER('"+loginId+"') AND LOGINASROLEID = '"+roleId+"'";
			ResultSet rs = con.select(strSQL);
			if(rs.next()){
...
//	FOR CUSTOM REPORT CHANGED BY SUNEEL
	...
	public ArrayList<CommonBean> getRefNoSearch(String refnumber,String destinationmarking){
		...
		String strSQL ="SELECT A.REFNO,A.REFID,A.REFERENCENAME,"
				...
				+ "REPLACE(REPLACE(A.SUBJECT, CHR(10), ' '), CHR(13), ' ') SUBJECT,"
				+ "'' STATUS,A.VIPSTATUS,"
				+ "C.STATENAME STATENAME,A.SUBJECTCODE,TO_CHAR(A.INCOMINGDATE,'DD/MM/YYYY') INCOMINGDATE "
	...
	public ArrayList<CommonBean> getFileNoSearch(String fileno,String roleId){
		...
		String strSQL = " SELECT DECODE(A.DESTINATIONMARKING,'"+roleId+"',A.REGISTRATIONNODES,A.REGISTRATIONNOORG) REGISTRATIONNO," +
				...
				"DECODE(A.DESTINATIONMARKING,'"+roleId+"',A.FILECOUNTERDES,A.FILECOUNTERORG) FILECOUNTER,A.FMID," +
				"(SELECT DISTINCT X.ROLENAME FROM MSTROLE X WHERE X.ROLEID = A.RECEIVEDFROM ) RECEIVEDFROM," +
				"DECODE(A.DESTINATIONMARKING,'"+roleId+"',A.FILESUBJECTCODEDES,A.FILESUBJECTCODEORG) FILESUBJECTCODE," +
				...
				" FROM TRNFILEHDR A WHERE UPPER(A.FILENO) = UPPER('"+fileno+"') AND A.FILESTATUS1 IN ('1','6','10','12')" +
...
public ArrayList<CommonBean> getRefDashBoardDetails(String loginId){
	...
	ArrayList<CommonBean> cm =(new CommonDAO_old()).getSQLResult(strSQL1, 1);
	String stForState = "";
