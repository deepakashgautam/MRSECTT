...
public class MstMessageDAO {
	static Logger log = LogManager.getLogger(MstMessageDAO.class);
...
//		String strSQL = " SELECT MESSAGETEXT,MESSAGETYPE,LOGINID,ISACTIVE FROM MSTMESSAGE  WHERE MESSAGETYPE='NT' AND ISACTIVE='Y'";
		...
		//	log.debug(strSQL);
			con.openConnection();
			PreparedStatement ps = null;
			ps = con.setPrepStatement(" SELECT NAME, TO_CHAR(TENURESTARTDATE,'DD-MON-YYYY') STARTDATE, NVL(TO_CHAR(TENUREENDDATE,'DD-MON-YYYY'),'Till Date') ENDDATE" +
					" FROM MSTTENURE A WHERE ROLEID = ? ORDER BY TENUREID DESC ");
...
//				bn.setField2(rs.getString("LOGINID"));
	...
	public ArrayList<CommonBean> getRefClassData(String roleID){
		...
		try {
			con.openConnection();
			PreparedStatement ps = null;
			ps = con.setPrepStatement(" SELECT TENUREID, LISTAGG(REFCLASS,', ') WITHIN GROUP (ORDER BY REFCLASS) REFCLASS FROM MSTCLASS WHERE ROLEID = ? GROUP BY TENUREID ORDER BY TENUREID DESC");
			ps.setString(1, roleID);
...
public ArrayList<CommonBean> getMenuLevel(String loginid){
	...
	try {
		con.openConnection();
		PreparedStatement ps = null;
		ps = con.setPrepStatement("SELECT MENUID FROM USERMENUTABLE WHERE GROUPID =? ");
	...
	public String saveMessage(MstRole bn)
	{
		...
		try
		{
			...
			else
			{
				...
				ps1 = con.setPrepStatement("INSERT INTO MSTROLE(ROLEID, ROLENAME) VALUES(?,?)");
				ps1.setString(1,bn.getROLEID());
				ps1.setString(2, bn.getROLENAME());
				ps1.executeUpdate();
	...
	public MstRole getRoleData(String roleid) {
		...
		try {
			...
			con.openConnection();
			PreparedStatement ps = null;
			ps= con.setPrepStatement(" SELECT ROLEID, ROLENAME FROM MSTROLE WHERE ROLEID = ?  ORDER BY ROLEID ");
			ps.setString(1, roleid);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
