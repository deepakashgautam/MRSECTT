...
function submitForm(btnclick) {
	...
	if("S"==btnclick){
		...
		&& chkblank(window.document.frmMstLogin.ISACTIVELOGIN)
...
function bodyOnLoad(){
	...
	showDiv();
	label2value();	
	setSessionSeachParams('sLOGINID', '<c:out value="<%=sBean.getField1()%>"/>');
	setSessionSeachParams('sLOGINNAME', '<c:out value="<%=sBean.getField2()%>"/>');
	setSessionSeachParams('sLOGINDESIGNATION', '<c:out value="<%=sBean.getField3()%>"/>');
	setSessionSeachParams('sROLEID', '<c:out value="<%=sBean.getField4()%>"/>');
 	getSearch();
}
function showDiv()
{
	...
	<tr>
		...
		<table id="data" width="61%">
					...
					<tr class="trodd">
						...
						<input type="text" name="LOGINID" id="LOGINID" value="<c:out value='<%=mstLoginBean.getLOGINID() %>'/>" maxlength="20" onblur="chkLoginID(this);" ></td>

<td nowrap valign="middle" width="121">Login Password<b><font
							color="red">*</font></b> :</td>
						<td valign="middle" width="124"><input type="password" name="PASSWORD" id="PASSWORD" maxlength="20"  size="20"></td>
					</tr>
					<tr class="trodd">
					<td width="147">User's Name<b><font color="red">*</font></b> :</td>
						<td width="182"><input type="text" name="LOGINNAME" id="LOGINNAME"
							value="<c:out value='<%=mstLoginBean.getLOGINNAME() %>'/>" maxlength="20"></td>
						<td nowrap valign="middle" width="121">User's Designation<b><font
							...
							value="<c:out value='<%=mstLoginBean.getLOGINDESIGNATION() %>'/>" size="20"></td>
					</tr>
					<tr class="trodd">
					...
					<tr class="trodd">
						<td nowrap valign="middle" width="147">Date(Starting)<b><font
							...
							<input type="text" name="STARTDATE" id="STARTDATE" maxlength="10" value="<c:out value='<%= mstLoginBean.getSTARTDATE() != null? mstLoginBean.getSTARTDATE():"" %>'/>" size="10"></td>
						<td nowrap valign="middle" width="121">Date(Ending) :</td><td valign="middle" width="124"><input type="text" name="ENDDATE" id="ENDDATE" size="10" value="<c:out value='<%= mstLoginBean.getENDDATE() != null? mstLoginBean.getENDDATE():"" %>'/>" maxlength="10"></td>
					</tr>
					<tr class="trodd">
					...
					<option value="<c:out value='<%=mstLoginBean2.getLOGINASROLEID() %>'/>" selected="selected"><c:out value='<%=mstLoginBean2.getLOGINASROLENAME() %>'/></option>
					...
					<option value="<c:out value='<%=beanCommon.getField1()%>'/>"
						<%=beanCommon.getField1().equalsIgnoreCase(mstLoginBean.getLOGINASROLEID())?"selected":""%>><c:out value='<%=beanCommon.getField2()%>'/></option>
					<%} 
					}
					%>
				</select></td>
