...
function reset()
{
	...
	//Set REF Class
	var refClass= new Array("0" <% for(int i=0;i<REFCLASS.length;i++){%>,"<c:out value='<%= REFCLASS[i] %>'/>"<%} %>);
	mulitselect(document.getElementById("REFCLASS"),document.getElementById("SEL_REFCLASS"),refClass);
	
	//Set REF Category
	var refCategory= new Array("0" <% for(int i=0;i<REFCATEGORY.length;i++){%>,"<c:out value='<%= REFCATEGORY[i] %>'/>"<%} %>);
	mulitselect(document.getElementById("REFCATEGORY"),document.getElementById("SEL_REFCATEGORY"),refCategory);

	//Set Subject Type
	var subType= new Array("0" <% for(int i=0;i<SUBJECTTYPE.length;i++){%>,"<c:out value='<%= SUBJECTTYPE[i] %>'/>"<%} %>);
	mulitselect(document.getElementById("SUBJECTTYPE"),document.getElementById("SEL_SUBJECTTYPE"),subType);

	//Set Subject Type
	var markTo= new Array("0" <% for(int i=0;i<MARKTO.length;i++){%>,"<c:out value='<%= MARKTO[i] %>'/>"<%} %>);
...
//	var seltabOrderBy = new Array("0"  for(int i=0;i<seltabOrderBy.length;i++){%>," = seltabOrderBy[i] "} %>);
	...
	var INDATECONDITION = "<c:out value='<%=INDATECONDITION%>'/>";
	...
	var LETDATECONDITION = "<c:out value='<%=LETDATECONDITION%>'/>";
	...
	var ACKNDATECONDITION = "<c:out value='<%=ACKNDATECONDITION%>'/>";
	...
	var MARKINGDATECONDITION = "<c:out value='<%=MARKINGDATECONDITION%>'/>";
	...
	var TARGETDATECONDITION = "<c:out value='<%=TARGETDATECONDITION%>'/>";
	...
	var SIGNEDONCONDITION = "<c:out value='<%=SIGNEDONCONDITION%>'/>";
...
//		fnSelectAll("SEL_ROLEID");
		...
		fnSelectAll("SEL_COMPLIANCEREMARKS");
		fnSelectAll("SEL_REFCATEGORY");
		fnSelectAll("SEL_SUBJECTTYPE");
		fnSelectAll("SEL_MARKTO");
		fnSelectAll("sel_tabcols");
...
//	ArrayList<CommonBean> refCategoryList = (new CommonDAO()).getSQLResult(refCategorySQL, 1);
	...
	String refLanguage = "SELECT CODE, FNAME, CODETYPE FROM MSTGEC WHERE CODETYPE = '3' AND CODE<>'00'"; 
	ArrayList<CommonBean> refLanguageList = (new CommonDAO()).getSQLResult(refLanguage, 3);
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr class="firstbg">
														...
														<table width="100%" border="0">
															<tbody>
																<tr class="trodd">
																	...
																	<td align="left" width="130"> Language</td>
																	<td colspan="3" align="left" width="327">
																	<select name="LANGUAGE" id="LANGUAGE">
																		<option value="">- SELECT -</option>
																	<%
																		for(int i=0;i<refLanguageList.size();i++){
																		CommonBean beanCommon=(CommonBean) refLanguageList.get(i);
																	%>
																		<option value="<c:out value='<%= beanCommon.getField1()%>'/>" <%=beanCommon.getField1().equalsIgnoreCase(LANGUAGE)?"selected":""%>><c:out value='<%= beanCommon.getField2()%>'/></option>
																	<%}%>
																	</select>
																	</td>
																</tr>
																<tr class="treven">
																	...
																	</select>&nbsp;<input type="text" name="INDATEFROM" id="INDATEFROM" size="10" value="<c:out value='<%=sessionBean.getTENURESTARTDATE() %>'/>"/>
																	&nbsp;<input type="text" name="INDATETO" id="INDATETO" size="10" value="<c:out value='<%= serverDate %>'/>"
																		/></td>
																	...
																	&nbsp;<input type="text" name="LETDATETO" id="LETDATETO" size="10"
																		style="display: none;" value="<c:out value='<%= LETDATETO%>'/>" /></td>
																</tr>
																<tr class="trodd">
																	...
																	<td nowrap="nowrap" align="left" width="351"><input style=" background-image: url('images/search.png'); background-position: right;background-repeat: no-repeat;"
																		type="text" id="RECEIVEDFROM" name="RECEIVEDFROM" size="34" value="<c:out value='<%= RECEIVEDFROM %>'/>"   /></td>
																	<td align="left" width="130">Status</td>
																	<td colspan="3" align="left" width="327"><input value="<c:out value='<%= STATUS%>'/>" style=" background-image: url('images/search.png'); background-position: right;background-repeat: no-repeat;"
																		type="text" name="STATUS" id="STATUS" size="34" /></td>
																</tr>
																<tr class="treven">
																	<td width="124" align="left">State</td>
																	<td nowrap="nowrap" align="left" width="351"><input style=" background-image: url('images/search.png'); background-position: right;background-repeat: no-repeat;"
																		type="text" name="STATE" id="STATE" size="34" value="<c:out value='<%= STATE%>'/>"/></td>
																	<td align="left" width="130">Subject</td>
																	<td colspan="3" align="left" width="327"><input
																		type="text" name="SUBJECT" id="SUBJECT" size="34" value="<c:out value='<%= SUBJECT%>'/>" /></td>
																</tr>
																<tr class="trodd">
																	<td width="124" align="left">Ack. By</td>
																	<td align="left" width="351"><input type="text"
																		name="ACKNBY" id="ACKNBY" size="34" value="<c:out value='<%= ACKNBY%>'/>" /></td>
																	...
																	<td colspan="3" align="left" width="327"><select
																		...
																		value="<c:out value='<%= ACKNDATEFROM%>'/>" />
																	&nbsp;<input type="text" name="ACKNDATETO" id="ACKNDATETO" size="10"
																		style="display: none;" value="<c:out value='<%= ACKNDATETO%>'/>" /></td>
																</tr>
																<tr class="treven">
																	...
																	<td align="left" width="351"><select
																		...
																		size="10" value="<c:out value='<%= MARKINGDATEFROM%>'/>" />
																	&nbsp;<input type="text" name="MARKINGDATETO" id="MARKINGDATETO" size="10"
																		style="display: none;" value="<c:out value='<%= MARKINGDATETO%>'/>" /></td>
																	...
																	<td colspan="3" align="left" width="327"><select
																		...
																		size="10" id="TARGETDATEFROM" value="<c:out value='<%= TARGETDATEFROM%>'/>" />
																	&nbsp;<input type="text" name="TARGETDATETO" id="TARGETDATETO" size="10"
																		style="display: none;"  value="<c:out value='<%= TARGETDATETO%>'/>" /></td>
																</tr>
																<tr class="trodd">
																	<td width="124" align="left">Remarks</td>
																	<td align="left" width="351"><input type="text"
																		name="REMARKS" id="REMARKS" size="34" value="<c:out value='<%= REMARKS%>'/>" /></td>
																...
																<select	name="SIGNEDBY" id="SIGNEDBY">
																	...
																	%>
																		<option value="<c:out value='<%= beanCommon.getField2()%>'/>" <%=beanCommon.getField2().equalsIgnoreCase(SIGNEDBY)?"selected":""%>><c:out value='<%= beanCommon.getField1()%>'/></option>
																	<%}%>
																</select></td>
																</tr>
																<tr class="treven">
                  		...
                  		<select
                  										...
                  										<td > 
                  										<select
															ondblclick="fnMoveItems('COMPLIANCEREMARKS', 'SEL_COMPLIANCEREMARKS');"
															name="COMPLIANCEREMARKS" id="COMPLIANCEREMARKS" multiple="multiple"
															style="width: 180px; height: 100px">
															<%
															for(int i=0;i<complianceCode.size();i++){
															CommonBean beanCommon=(CommonBean) complianceCode.get(i);
							%>
                  		...
                  		<td valign="middle" align="center"> <img
															align="middle" src="images/next.gif"
															onclick="fnMoveItems( 'COMPLIANCEREMARKS','SEL_COMPLIANCEREMARKS')" /> <br />
														<img align="middle" src="images/prev.gif"
															onclick="fnMoveItems( 'SEL_COMPLIANCEREMARKS','COMPLIANCEREMARKS')" /><br /> 
                  		</td>
                  		<td>
                  		<select
															ondblclick="fnMoveItems('SEL_COMPLIANCEREMARKS', 'COMPLIANCEREMARKS');"
															name="SEL_COMPLIANCEREMARKS" id="SEL_COMPLIANCEREMARKS" multiple="multiple"
															style="width: 180px; height: 100px">
...
<!-- 												<option value="1">Link Reference ID</option>    -->
												<option value="2">Reference No</option>                          
												<option value="3">Incoming Date</option>                   
												<option value="4">Letter Date</option>
												<option value="46">Letter No</option>                     
												<option value="5">Received From</option> 
												<option value="45">Status/Party</option>                 
												<option value="6">Status</option>                      
												<option value="7">State</option> 
												<option value="8">Party</option>                       
												<option value="9">Sub.</option>                    
										...
										<% String frwrdTo = "GETROLENAME((SELECT MARKINGTO FROM TRNMARKING WHERE REFID = TRNREFERENCE.REFID AND MARKINGSEQUENCE = 1)) "; %>
												<option value="11">Forward To</option>                      
												<option value="12">Forward Date</option>                    
												<option value="13">Target Date</option>                     
												<option value="14">Remarks</option>                        
										<% String sgndBy = "(SELECT  A.ROLENAME FROM MSTROLE A WHERE A.ROLEID = TRNREFERENCE.SIGNEDBY) "; %>
												<option value="15">Signed By</option>                       
												<option value="16">Signed On</option>                       
										<% String acknBy = "(SELECT A.ROLENAME FROM MSTROLE A WHERE A.ROLEID = TRNREFERENCE.ACKBY) "; %>
												<option value="17">Acknowledge By</option>                          
												<option value="18">Acknowledge Date</option>                        
										<% String markTo = "(SELECT A.ROLENAME FROM MSTROLE A WHERE A.ROLEID = TRNREFERENCE.IMARKINGTO) "; %>
												<option value="19">Mark To</option>
												<option value="20">FM Regn.</option>
												<option value="21">FM Date</option>
												<option value="22">File No.</option>
										<% String fStatus1 = "(SELECT FNAME FROM MSTGEC WHERE CODETYPE = '6' AND CODE<>'00' AND CODE = TRNREFERENCE.FILESTATUS1) "; %>
												<option value="23">File Status 1</option>
										<% String fStatus2 = "(SELECT FNAME FROM MSTGEC WHERE CODETYPE = '7' AND CODE<>'00' AND CODE = TRNREFERENCE.FILESTATUS2) "; %>
												<option value="24">File Status 2</option>
										<% String rtrnTo = "(SELECT A.ROLENAME FROM MSTROLE A WHERE A.ROLEID = TRNREFERENCE.DMARKINGTO) "; %>
												<option value="25">Return To</option>
												<option value="26">Return On</option>
												<option value="27">Reply</option>
												<option value="28">Is Budget</option>
												<option value="29">VIPTYPE</option>
												<!-- <option value="30">Keyword 1</option>
												<option value="31">Keyword 2</option>
												<option value="32">Keyword 3</option> -->
												<option value="33">MOBILENO</option>
												<option value="34">VIPCONSTITUENCY</option>
												<option value="35">Compliance</option>
												<option value="36">Compliance Remarks</option>
												
												<option value="37">Compliance Date</option>
												<option value="38">Tour/Parliament Remarks</option>
												
												
												
												<option value="39">Ref Currently With</option>
											    <!-- <option value="40">File Currently With</option> -->
					                  		    <option value="41">Ref Movement</option>
					                  		  <!--   <option value="42">File Movement</option> -->
					                  		     <option value="43">E-Office Ref No</option>
...
-->
                 ...
                 <br />
                  <table border="1" bordercolor="#7e7d7d" cellspacing="0" cellpadding="0" style="border-collapse:collapse; border: solid 1px #7e7d7d;">
                  	<tr class="tab1">
              			<th>   Report  Title</th><td> <input type="text" name="repheader" id="repheader" value="<c:out value='<%= repheader %>'/>" size="100" /></td>
              		</tr>
