...
ArrayList<CommonBean> RefDashBoardDetails = new DashboardSubjectWiseDAO().getRefDashBoardDetailList(subjectCode,loginID,status);
...
/* 
function SaveCompliance(refno,sno){
	//alert()
	//alert(refno);
	...
	DashboardSubjectWiseDAO.saveComplianceData(refno,compliance,compliianceRem,dateSaveCompliance);
...
<!-- CHANGED ON 24_07_2012 -->
			<table width="100%" style="margin-top: 1px;margin: 1px;" border="1" id="example">
				...
				<td style="border-bottom: dotted; border-width: 1px; border-color: blue"><b>State</b></td>
				...
				<%
					for(int i=0;i<RefDashBoardDetails.size();i++){
						...
						<td><c:out value='<%=bn.getField1() %>' />&nbsp;<br> <c:out value='<%=bn.getField2() %>'/></td>
						<td ><c:out value='<%=bn.getField3() %>' /> &nbsp;</td>
						<td><c:out value='<%=bn.getField4() %>' />&nbsp;</td>
						<td><c:out value='<%=bn.getField19() %>' />&nbsp;</td>
						<td><c:out value='<%=bn.getField18() %>' />&nbsp;</td>
						
						
						<td><c:out value='<%=bn.getField5() %>' />&nbsp;</td>
						<td><c:out value='<%=bn.getField6() %>' />&nbsp;<br> <c:out value='<%=bn.getField7() %>' /></td>
						<td><c:out value='<%=bn.getField8() %>' />&nbsp;<br> <c:out value='<%=bn.getField9() %>' /><br> <c:out value='<%=bn.getField10() %>' /></td>
					
						<td><c:out value='<%=bn.getField13() %>' />&nbsp;</td>
						
						<td><c:out value='<%=bn.getField14() %>' />&nbsp;
						...
						<td>
						<%-- <textarea id ="complianceRemarks_<%=i+1%>" onkeypress="allowAlphaNum('()/*');" rows="3" cols="20"> --%>
						<c:out value='<%=bn.getField15()%>' />&nbsp;
						<!-- </textarea> -->
						</td>
						<td align="center" ><img border="0" alt="Image Gallery" src="${pageContext.request.contextPath}/images/stn.gif" onclick="popupGallery('<c:out value="<%= bn.getField17()%>" />');"></td>
					<%-- 	<td><input type="button" name="SaveCompliance111" value="save" onclick="SaveCompliance('<%=bn.getField1() %>',<%=i+1 %>);"
