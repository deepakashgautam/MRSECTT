			...
			<div id="tabs" style="border: 0px">
				...
				<%} %>
			    		...
			    		<tr class="trodd">
							...
							<td colspan="2"><input type="text" id="FILENO" onkeydown="changecase(this,'U')" size="40" name="FILENO" tabindex="1" value="<%= fileBean.getFILENO()%>" onblur="getFileSearch(this);setOraignatedAt(this);" onkeypress="allowAlphaNum('()&/*-');changecase(this,'U')">
						...
						<td nowrap >E-Office File No.</td>							 
								<td nowrap colspan="2"><input type="text" name="EofficeFILENO" id="EofficeFILENO" size="40" tabindex="4" maxlength="40" style="text-transform: uppercase;"
								 onkeypress="allowAlphaNumWithoutSpace('/');" value="<%=fileBean.getEOFFICEFILENO()%>"></td>
					...
					TrnFileRef refBean = (TrnFileRef)fileBean.getRefBeanList().get(i);
						...
						name="REFNO" id="REFNO" value="<%=refBean.getREFNO()+" ("+refBean.getINCOMINGDATE()+")" %>" style=" background-image: url('${pageContext.request.contextPath}/images/search.png'); background-position: right;background-repeat: no-repeat; ">
								...
								<%
									...
									CommonBean beanCommon = (CommonBean) complianceCode.get(k);
									%>
								<option value="<%=beanCommon.getField1()%>" 	<%=beanCommon.getField1().equals(refBean.getCompliance())?"selected":""%>><%=beanCommon.getField2()%></option>
								<%
								}%>
							</select>
									...
									<td><textarea rows="2"
											cols="40" name="COMPREMARKS" id="COMPREMARKS" onkeypress="allowAlphaNum('()&/*');" tabindex="7"><%= refBean.getComplianceremarks()%></textarea></td>
...
<%
						...
						%>
									...
									<tr class="trodd">
										<td nowrap="nowrap" width="80"> <textarea rows="2"
											cols="80" name="SUBJECT" id="SUBJECT" onkeypress="allowAlphaNum('()&/*');" tabindex="8"><%= fileBean.getSUBJECT()%></textarea>
							...
							%>
									...
									<%=beanCommon.getField1().equalsIgnoreCase(markingBeanU.getMARKINGTO())?"selected":""%>><%=beanCommon.getField2()%></option>
										...
										<textarea rows="2" cols="20" name="UMARKINGREMARK" id="UMARKINGREMARK"  tabindex="13"><%= markingBeanU.getMARKINGREMARK()%></textarea></td>
				...
				<tbody>
			    	...
			    	<tr class="trodd">
							...
							<td id="tdn1" style="display: none"><div id="tdSentTo" style="display: none;"><input type="text"  name="SENTTO" id="SENTTO" value="<%= fileBean.getSENTTO().length()>0? fileBean.getSENTTO():"" %>" tabindex="19"></div></td>
							</tr>
				</tbody>	
				</table>
				</fieldset>
