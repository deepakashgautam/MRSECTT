...
function printme(obj,x)
{
 ...
 function SaveFormat()
 {
 	...
 	CustomReportDAO.updateReportLayout("<%=reportid%>",layout,colwidth,
 ...
 {
 	var type= num==1?'R':'F';
 	var url="ImageGallery.jsp?refId1="+refId+"&flgnoenc="+flg+"&type="+type;
 ...
 function downloadAllAttachment(){
	    ...
	    var listCheckbox=document.querySelectorAll('input[type=checkbox]:not(:checked)');
	    var l=listCheckbox.length
		 console.log("size "+l);
	    if(l<=30){
	       listCheckbox.forEach(function myfunction(item,index){
				
	          ((index==(l-1))?(list+=item.value+" "):(list+=item.value+", "));
				 
	          })
				 
	          
	          console.log("List: "+list)
	          CustomReportDAO.getFileName(list,
		                   function(data)
		                  {   
		                     ...
		                     data.forEach(function myfunction(item,index){
		                    	 ...
		                    	 window.open("${pageContext.request.contextPath}/DownloadAllAttachments?filenames="+item.GENFILENAME+"&types="+item.type,'_blank','width=900,height=800'); 
      ...
      <tr>
        ...
        <table width="984px" border="1" class="bor" cellspacing="0" cellpadding="0" style="border-collapse:collapse; margin-top:7px;">
          <tr>
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="tablehead"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <th class="tableheadtext" style="font-size: 12px;">  <%= mstarr.get(0).getHeader() %></th>
                  </tr>
                </table></td>
              </tr>
              <tr><td></td><tr>
              ...
              <span style="display: <%= "Y".equalsIgnoreCase(save)?"block":"none"%>">
			</span>
			<input type="radio" <%=(mstarr.get(0).getLayout().length()==0 ||mstarr.get(0).getLayout().equalsIgnoreCase("P"))?"checked":""%> name="layout" name="layout" value="P"  onclick="setlayout(this);"> Portrait<br>
										<input <%=mstarr.get(0).getLayout().equalsIgnoreCase("L")?"checked":""%> type="radio" name="layout" value="L" onclick="setlayout(this);"> Landscape
			
			<input type="button" value="Back to Report List" class="butts" onclick="window.location.href='CustomReport.jsp'" />
			...
			<input type="button" style="display:none" value="Download"  onclick="downloadAllAttachment()"/>
                  	...
                  	<%
						 ...
						 %>
						<br/>
					<table border="1" id="sorttable" bordercolor="#7e7d7d" cellspacing="0" cellpadding="0" 
					width="<%=mstarr.get(0).getLayout().equalsIgnoreCase("L")?"850":"650" %>" >
							<% if(bean.hasData()){ 
								arrh = bean.getArrh();
								arrDT = bean.getArrDT();
								arr = bean.getArr();
						...
						<thead style="display: table-header-group;">
							...
							<th style="font-size: 11px; font: Tahoma;" id="col_0" <%=mstarr.get(0).getColwidth().length()>=1?("width='"+mstarr.get(0).getColwidth().split("~")[0]+"'"):"" %> valign="top" align="center"> S.No. </th>							
								<%for(int i=0;i< arrh.size();i++){ %>
									<th <%=mstarr.get(0).getColwidth().length()>=(i+1)?("width='"+mstarr.get(0).getColwidth().split("~")[i+1]+"'"):"" %> style="font-size: 11px; font: Tahoma;" id="col_<%=(i+1)%>" valign="top" align="center" > <%= arrh.get(i).toString()%> </th>
								<%} %>
							</tr>
						</thead>
						<tbody>
							<%	int cnt=1;
								...
								<tr class="<%= i%(2*arrh.size())==0?"trodd":"treven" %>">
									<TD style="padding-left: 2px; padding-right: 2px; font-size: 11px; font: Tahoma;" align="right" valign="top"> <%= cnt++ %></TD>
								<%}  if((i+1)%(arrh.size())!=0){%>
								
									<TD valign="top" style="padding-left: 2px; padding-right: 2px; font-size: 11px; font: Tahoma;" <%= arrDT.get(i%arrh.size()).toString().equalsIgnoreCase("NUMBER")||arrDT.get(i%arrh.size()).toString().equalsIgnoreCase("DECIMAL")?"align='right'":"" %> >   <%= arr.get(i).toString()%>    </TD>
								<%} if((i+1)%(arrh.size())==0){%>	
								<TD valign="top" style="padding-left: 2px; padding-right: 2px; font-size: 11px; font: Tahoma;" <%= arrDT.get(i%arrh.size()).toString().equalsIgnoreCase("NUMBER")||arrDT.get(i%arrh.size()).toString().equalsIgnoreCase("DECIMAL")?"align='right'":"" %> >   <%=arr.get(i).toString().equalsIgnoreCase("1") ? "<img border='0' alt='Image Gallery' src='images/stn.gif' onclick='popupGallery("+arr.get(i-1).toString()+","+arr.get(i-1).toString()+",1);'> <input class='pdfcheckbox' style='display:none' type='checkbox'  value="+arr.get(i-1).toString()+" id="+arr.get(i-1).toString()+">":"" %> </TD>
								</tr>
								<%} %>							
