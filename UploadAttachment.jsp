<%@page import="in.org.cris.mrsectt.dao.UploadAttachmentDAO"%><html>
...
<%
    ...
    {
		dataArr = (new UploadAttachmentDAO()).getData(diarynofrom, diarynoto, datefrom, dateto, sessionBean.getLOGINASROLEID(), sessionBean.getISCONF()); 
	}
...
{
	var url="ImageGallery.jsp?refId="+refId+"&type=R";
...
<body>
<form name="frmUpload" id="frmUpload" action="UploadAttachment.jsp" method="post">
<table>
<tr>  
      <td > 
      		<font size="3" > 
      			<b><i>Reference-Create</i> - Upload
      		Attachment</b></font><img src="${pageContext.request.contextPath}/images/arrow_big.gif" width="6" height="11">    
				...
				<table width="100%" id="data" align="center">
								<tbody>
									...
									<td align="left" width="784"><input type="text" name="diarynofrom" id="diarynofrom" size="15" style="text-transform: uppercase;" value="<c:out value='<%= diarynofrom %>'/>" onkeyup="setToField(this, 'diarynoto');"> to&nbsp;
									<input type="text" name="diarynoto" id="diarynoto" size="15" value="<c:out value='<%= diarynoto %>'/>" style="text-transform: uppercase;"> (e.g. A/1)
									<input type="hidden" name="btnClick" id="btnClick" value="<c:out value='<%=request.getParameter("btnClick")!=null ? request.getParameter("btnClick"):""%>'/>" /></td>
									</tr>
						...
						<table class="tablesorter" id="sorttable" align="center">
							...
							%>
								<tr>
									...
									<td align="left" ><c:out value='<%= bean.getField2() %>'/>
									<input type="hidden" name="refId" id="refId" value="<c:out value='<%= bean.getField1() %>'/>" readonly="readonly" size="6"></td>
									<td align="left" ><c:out value='<%= bean.getField3() %>'/></td>
									<td align="left" ><c:out value='<%= bean.getField4() %>'/></td>
									<td align="left" ><c:out value='<%= bean.getField5() %>'/></td>
									<c:set var="field1" value="<%= bean.getField1() %>"/>
									<td align="center" ><img border="0" alt="Image Gallery" src="${pageContext.request.contextPath}/images/stn.gif" onclick="popupGallery('<c:out value="${enc:encrypt(field1)}" />');"></td>
								</tr>
							<%} %>
						</table>
						</div>
