...
contentsource: ["smoothcontainer", "MENU1.jsp"] //"markup" or ["container_id", "path_to_menu_file"]
});
...
{
	if(x=='home')
			document.getElementById("tmscontent").src="<%= request.getContextPath().toString() %>/RefrenceDashboardSubjectWise_Other.jsp";
	else if(x=='logoff')
		window.open("LogOffController","_self");
	else if(x=='pass')
		document.getElementById("tmscontent").src="<%= request.getContextPath().toString() %>/UpdateProfile.jsp?open=self";
	else if(x=='help')
		{
			var ctx = "<%= request.getRequestURL() %>";
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" style="margin-top: 0">
	<tr height="100%"  align="right">
		<td background="${pageContext.request.contextPath}/images/header<%= theme%>.jpg" width="100%" valign="top" height="100%" style="background-repeat: no-repeat; size: 100% auto;">
			<table width="100%" height="70" cellpadding="0" cellspacing="0" >
				...
				<td align="right" valign="top" nowrap="nowrap"><font color="white"><b><%= sessionBean.getLOGINNAME() %> (<%=sessionBean.getLOGINASROLENAME() %>)</b></font>
					</td>
...
<!--	<div class="TmsTitle"><marquee behavior="scroll" scrolldelay="1" scrollamount="3"><font color="red">Application will not be available from 07/08/2012:11:30 to 07/08/2012:14:00 due to maintenance purpose.</font></marquee></div> -->
		<div class="TmsTitle" align="center"><b><%= sessionBean.getTENUREOFFICENAME() %></b></div>
	        			</td>
