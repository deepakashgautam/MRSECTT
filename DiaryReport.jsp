...
<%
	...
	String subjectcode = request.getAttribute("subjectcode") != null ? (String)request.getAttribute("subjectcode"):"";
	if(subjectcode.length()>0){
		String subCode = "SELECT SUBJECTNAME FROM MSTSUBJECT WHERE SUBJECTCODE = '"+subjectcode+"'"; 
		ArrayList<CommonBean> subName = (new CommonDAO()).getSQLResult(subCode, 1);
		subjectcode = subName.get(0).getField1();
	}
	
	String SEARCHFOR = request.getAttribute("searchfor") != null ? (String)request.getAttribute("searchfor"):"";
			SEARCHFOR = SEARCHFOR.length() > 0 ? SEARCHFOR+", " :"";
			
	String RECEIVEDFROM = request.getAttribute("receivedfrom") != null ? (String)request.getAttribute("receivedfrom") :"";
			RECEIVEDFROM = RECEIVEDFROM.length() > 0 ? RECEIVEDFROM+", " :"";
	...
	String SUBCODE = subjectcode.length() > 0 ? subjectcode+", " :"";
		   SUBCODE = SUBCODE.length() > 0 ? SUBCODE+", " :"";
		   
	String markto = request.getAttribute("markto") != null ? (String)request.getAttribute("markto") :"";
	if(markto.length()>0){
		String roleName = "SELECT ROLENAME FROM MSTROLE WHERE ROLEID = '"+markto+"'"; 
		ArrayList<CommonBean> rName = (new CommonDAO()).getSQLResult(roleName, 1);
		markto = rName.get(0).getField1();
	}
	String tagTMP = (String)request.getAttribute("tag");
			tagTMP = tagTMP.length()>0 ? tagTMP+", " :"";
	
	String fdiarynoTMP = request.getAttribute("diarynofrom") != null?(String)request.getAttribute("diarynofrom"):"";
			fdiarynoTMP = fdiarynoTMP.length()>0 ? fdiarynoTMP+" - " :"";

	String tdiarynoTMP = request.getAttribute("diarynoto") != null?(String)request.getAttribute("diarynoto"):"";
//	String TDIARYNO = tdiarynoTMP.length()>0 ? tdiarynoTMP+", " :"";
	
	
	String paramSTR1 = REFCLASS+SEARCHFOR+RECEIVEDFROM+SUBCODE+tagTMP+markto+fdiarynoTMP+tdiarynoTMP;
...
<table width="100%" border="0" cellspacing="0" bordercolor="black">
		...
		<tr valign="top">
			...
			<td align="center" style="border-bottom: dotted; border-top: dotted; border-width: 1px; border-color: gray; font-family: Tahoma; font-size: 10"><b>Ref. No<br>Incoming Dt.<br>eOffice No</b></td>
			<td align="center" style="border-bottom: dotted; border-top: dotted; border-width: 1px; border-color: gray; font-family: Tahoma; font-size: 10"><b>Ref. Dated<br>Letter No</b>
			</td>
		...
		<tr>
		   	...
		   	<td align="center" style=" font-family: Tahoma; font-size: 10"><c:out value='<%=bean.getField1()%>'/><br><c:out value='<%=bean.getField3()%>'/><br><c:out value='<%=bean.getField33()%>'/></td>
		   	<td align="center" style=" font-family: Tahoma; font-size: 10"><c:out value='<%=bean.getField5()%>'/><br><c:out value='<%=bean.getField32()%>'/>&nbsp;</td>
