...
<%
	...
	String REFID = StringFormat.nullString( (String)request.getAttribute("refId"));
	String refId = request.getParameter("refId")!=null ? request.getParameter("refId") : (request.getAttribute("refId")!=null ? request.getAttribute("refId").toString() : "");
	...
	String flg = request.getParameter("flg")!=null ? request.getParameter("flg") : (request.getAttribute("flg")!=null ? request.getAttribute("flg").toString() : "");
...
//	if(tmparray[k].split("~")[1]=="jpg")
		iWebAlbumPhotos[k]={"caption": ""+tmparray[k].split("~")[0], "url": ""+ip+"/ImageServlet?refId=<%=refId%>&file="+tmparray[k].split("~")[0], "comment": ""};
//	else
//		iWebAlbumPhotos[k]={"caption": ""+tmparray[k].split("~")[0]+".pdf", "url": ""+ip+"/ImageServlet?refId=<%=refId%>&file="+tmparray[k]+".pdf", "comment": ""};
}
				...
				<table border="0" cellspacing="0" cellpadding="0" class="iwebalbum_photobox_table">
					<tr>
						<td class="iwebalbum_photobox_table" align="center" valign="middle"> 
							...
							<a href="#">
							<img src="ImageServlet?refId=<%=refId %>&file=<%=arr.get(i).toString().split("~")[0] %>_s.jpg&type=<%=type%>" alt="<%=arr.get(i).toString().split("~")[0] %>.jpg" onclick="window.open('ImageServlet?refId=<%=refId %>&file=<%=arr.get(i).toString().split("~")[0] %>.jpg&type=<%=type%>','','toolbar=0,status=0,location=0,menubar=0,directories=0,titlebar=0,left=0,top=0,scrollbars=1,resizable=1');"  class="photobox" width="90" height="90" /></a>
							<%} else { %>
							<a href="#">
							<img src="${pageContext.request.contextPath}/images/pdf_icon.png" width="90" height="90" alt="<%=arr.get(i).toString().split("~")[0] %>.pdf" onclick="window.open('ImageServlet?refId=<%=refId %>&file=<%=arr.get(i).toString().split("~")[0] %>.pdf&type=<%=type%>','','toolbar=0,status=0,location=0,menubar=0,directories=0,titlebar=0,left=0,top=0,scrollbars=1,resizable=1');" class="photobox" /></a>
							<%} %>
<%
