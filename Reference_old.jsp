			...
			<div id="tabs" style=" border: 0px">
			    ...
			    <div id="MainReferences" >
					...
					<tr>
						...
						<td nowrap colspan="2"> 
						<input type="text" name="EOFFICEREFNO" id="EOFFICEREFNO" value="<%= refBean.getEOFFICEREFNO()%>" />
							<input type="hidden" name="EOFFICEREFNOForSave" id="EOFFICEREFNOForSave" value="<%= refBean.getEOFFICEREFNO()%>" />
						...
						<%
								...
								%>
						<font color='green'><%= refBean.getEOFFICEMSG()%></font>
						<%
									}else{
								%>
						<font color='red'><%= refBean.getEOFFICEMSG()%></font>
							<%
									}
								%>	
						</td>
...
<option value="<%=beanCommon.getField1()%>" <%=beanCommon.getField1().equalsIgnoreCase(refBean.getREFCLASS())?"selected":""%><%=((request.getAttribute("refBean")==null)&&(request.getParameter("REFCLASS")!=null ?request.getParameter("REFCLASS"):"").equalsIgnoreCase(beanCommon.getField1()) ?"selected":"" ) %>><%=beanCommon.getField1()%></option>
						...
						<tr >
							...
							<td nowrap="nowrap"><select name="URGENCY" style="width: 120px" tabindex="3"  >
								...
								<%
										...
										%>
								<option value="<%=beanCommon.getField1()%>" <%=beanCommon.getField1().equalsIgnoreCase(refBean.getURGENCY())?"selected":""%><%=((request.getAttribute("refBean")==null)&&"G".equalsIgnoreCase(beanCommon.getField1()) ?"selected":"" ) %> <%=beanCommon.getField1().equalsIgnoreCase(refBean.getURGENCY())?"selected":""%><%=((request.getAttribute("refBean")!=null)&& refBean.getURGENCY().equalsIgnoreCase("") && "G".equalsIgnoreCase(beanCommon.getField1()) ?"selected":"" ) %>><%=beanCommon.getField2()%></option>
								<%
									}%>
							</select></td>
			    	...
			    	<table id="data" width="100%">	
			    		<tr >
			    			...
			    			<td width="303">
								...
								<input type="hidden" name="ISCONF" id="ISCONF" value="<%= sessionBean.getISCONF() %>" size="2">
									...
									<input type="hidden" name="TENUREID" id="TENUREID" value="<%= refBean.getTENUREID() %>">
			    			</td>
						...
						<select name="ISBUDGET" style="width: 160px" onchange=setMaxRowsOfMarking(this); tabindex="7"  >
							<option value=""> </option>
							<option value="N" <%="N".equalsIgnoreCase(refBean.getISBUDGET())?"selected":""%> <%=((request.getAttribute("refBean")==null) ?"selected":"" ) %> <%=((eofficeFlag=="eofficedata") ?"selected":"" ) %>>Non-Budget</option>
					...
					<td>Party</td>
						...
						<option value="" selected> </option>
								<%
									...
									<option value="<%=beanCommon.getField1()%>" <%=beanCommon.getField1().equalsIgnoreCase(refBean.getSALUTATION())?"selected":""%><%=((request.getAttribute("refBean")==null)&&"PO".equalsIgnoreCase(beanCommon.getField1()) ?"selected":"" ) %>><%=beanCommon.getField2()%></option>
								<%
									}%>
					</select></td>
						<td width="40">Address(English)</td><td nowrap="nowrap" width="200"><input type="text"
								...
								value="<%= refBean.getADDRESSENGLISH()%>" tabindex="9"
								onblur="window.document.getElementsByName('SUBJECT')[1].focus();" size="30" onkeypress="allowAlphaNum('()/');" ></td>
						<td><input type="hidden" name="VIPID" value="<%=refBean.getADDVIPID() %>"/></td>
						<td></td>
						<td><input type="hidden" name="VIPADDRESS" value="<%=refBean.getADDVIPTYPE() %>"/>
						<input type="hidden" name="VIPSTATUSDESC" value="<%=refBean.getVIPSTATUSDESC() %>"/>
						</td>
					</tr>
					...
					<tr  style="display: none">
						...
						<%
							...
							%>
								<option value="<%=beanCommon.getField1()%>"><%=beanCommon.getField2()%> ( <%=beanCommon.getField3()%> ) </option>
							<%
						}%>
					</select>
						</td>
						
						<td nowrap="nowrap" width="110"><input type="text" name="MARKINGDATE" id="MARKINGDATE" size="10" maxlength="10" value="<%=serverDate %>"  /><input type="hidden" id="servDate" value="<%=serverDate %>" /></td>
					...
					TrnMarking markingBean = (TrnMarking)refBean.getMarkingBeanList().get(i);
						...
						<tr >
							...
							<input type="hidden" name="MARKINGRECEIPTNUMBER" readonly="readonly" class="readonly" value="<%=markingBean.getEOFFICERECEIPTNO()%>" >
							
							
						<textarea rows="2" cols="70" onkeypress="allowAlphaNum('()&/*');"
											name="SUBJECT" id="SUBJECT" ><%= refBean.getSUBJECT()%></textarea>
						<br>
						<span style="display: none;">Stn1: <input type="text" name="keyword1" size="17" value="<%= refBean.getKeywords1()%>"></span>
						<span style="display: none;">Stn2: <input type="text" name="keyword2" size="17" value="<%= refBean.getKeywords2()%>"></span>
						<span style="display: none;">Trn: <input type="text" name="keyword3" size="17" value="<%= refBean.getKeywords3()%>"></span>	
						<span style="display: none;"> <input type="text" name="k1"  value="<%= refBean.getKeywords1()%>"> <input type="text" name="k2"  value="<%= refBean.getKeywords2()%>" > <input type="text" name="k3"  value="<%= refBean.getKeywords3()%>" > </span>						
											</td>
							<td nowrap="nowrap" width="133">
							<select name="SUBJECTCODE" id="SUBJECTCODE" style="width: 120px;" onclick="displayKeyword(this,true);"> 
							<option value="" selected> </option>
						...
						<%
							...
							%>
								<option value="<%=beanCommon.getField1()%>" <%=beanCommon.getField1().equalsIgnoreCase(markingBean.getMARKINGTO())?"selected":""%>><%=beanCommon.getField2()%> ( <%=beanCommon.getField3()%> )</option>
							<%
						}%>
					</select>
							</td>
							<td nowrap="nowrap" width="110"><input type="text" name="MARKINGDATE" id="MARKINGDATE" size="10" maxlength="10" value="<%= markingBean.getMARKINGDATE()%>"  ><input type="hidden" id="servDate" value="<%=serverDate %>" /></td>
				...
				<table width="100%" id="data">
					...
					<td nowrap="nowrap"  width="517">Tour/Parliament Remarks 
						<br>
							<textarea onkeypress="allowAlphaNum('()/*');" rows="2" cols="70"
								name="TOURREMARKS" tabindex="20"><%= refBean.getTOURREMARKS()%></textarea></td>
				...
				<table width="100%" id="data">
					...
					<td nowrap="nowrap" width="120">Compliance</td>
			    		<td width="216">
							...
							<option value="" selected>Select one- </option>
								<%
									...
									%>
								<option value="<%=beanCommon.getField1()%>" <%=beanCommon.getField1().equalsIgnoreCase(refBean.getCOMPLIANCE())?"selected":""%>><%=beanCommon.getField2()%></option>
								<%
								}%>
							</select>
						</td>
						...
						<br>
							<textarea onkeypress="allowAlphaNum('()/*');" rows="2" cols="70"
								name="COMPREMARKS" tabindex="20"><%= refBean.getCOMPLIANCEREMARKS()%></textarea></td>
			    	...
			    	<tr >
						...
						<td nowrap="nowrap" width="150"><input type="text" name="COMPDATE" id="COMPDATE" tabindex="24"
							value="<%= refBean.getCOMPLIANCEDATE()%>" onblur="chkWorkDate(this);"  
							onkeypress="chknumeric(47);" maxlength="10"></td>
					...
					<tr >
						<td align="left" width="100%" colspan="4">&nbsp;
							...
							<%} %>	
						    	...
						    	%>
								<tr>
									<td width="507" align="left"><a href="#" onclick="openAttachment('<%=cmnbn.getField1()%>','<%=cmnbn.getField2()%>','<%=cmnbn.getField3()%>','<%=cmnbn.getField4()%>','<%=cmnbn.getField5()%>');"><font color="blue"><%=cmnbn.getField3()%></font></a></td>
									<td align="center">
...
<%	if(msg.length()>3) { 
	...
	%>
			...
			<BR><img src="images/<%= msg.substring(0,3)%>.gif"/><%= s[0] %>
